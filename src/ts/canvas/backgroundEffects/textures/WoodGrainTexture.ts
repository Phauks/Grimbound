...
export class WoodGrainTextureStrategy extends BaseTextureStrategy {
  ...
  generate(context: TextureContext): TextureResult {
    ...
    const imageData = this.forEachCircularPixel(context, (x, y, _nx, _ny, index, data) => {
      ...
      const grain = Math.sin((ny * WOOD_GRAIN_TEXTURE.GRAIN_FREQUENCY + turb * WOOD_GRAIN_TEXTURE.TURBULENCE_INFLUENCE) * Math.PI);

      // Add ring pattern
      const rings = Math.sin(nx * WOOD_GRAIN_TEXTURE.RING_FREQUENCY + turb * WOOD_GRAIN_TEXTURE.RING_TURBULENCE) * WOOD_GRAIN_TEXTURE.RING_AMPLITUDE;

      let value = (grain * 0.5 + 0.5) * WOOD_GRAIN_TEXTURE.GRAIN_WEIGHT + rings * WOOD_GRAIN_TEXTURE.RING_WEIGHT + WOOD_GRAIN_TEXTURE.BASE_OFFSET;
