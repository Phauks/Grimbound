...
import type { BackgroundStyle, EffectsConfig, TextureConfig } from '../types/backgroundEffects.js';
 ...
 */
function _applyTint(ctx: CanvasRenderingContext2D, diameter: number, tintColor: string): void {
  const imageData = ctx.getImageData(0, 0, diameter, diameter);
  const data = imageData.data;
...
function generateWatercolorTexture(
  ...
  const _r = parseInt(baseColor.slice(1, 3), 16);
  const _g = parseInt(baseColor.slice(3, 5), 16);
  const _b = parseInt(baseColor.slice(5, 7), 16);
...
function generateLinenTexture(
  ...
  for (let y = 0; y < diameter; y++) {
    for (let x = 0; x < diameter; x++) {
      ...
      const value = (horizontal * 0.5 + vertical * 0.5) * 0.6 + 0.3 + variation;
