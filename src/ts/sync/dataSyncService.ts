...
export class DataSyncService {
  ...
  async checkForUpdates(): Promise<boolean> {
    ...
    try {
      ...
      if (currentVersion && VersionManager.isOlder(availableVersion, currentVersion)) {
        logger.info(
          'DataSyncService',
          `Cached version is newer: ${currentVersion} > ${availableVersion}`
        );
        this.currentStatus.state = 'success';
        this.currentStatus.availableVersion = null;
        this.emitEvent('success', this.currentStatus);
      ...
      logger.info(
        'DataSyncService',
        `Update available: ${availableVersion} (current: ${currentVersion || 'none'})`
      );
  ...
  private startPeriodicUpdateChecks(): void {
    ...
    logger.info(
      'DataSyncService',
      `Starting periodic update checks every ${CONFIG.SYNC.CHECK_INTERVAL_MS}ms`
    );
   ...
   */
  private emitEvent(type: SyncEventType, status: SyncStatus, data?: SyncEvent['data']): void {
