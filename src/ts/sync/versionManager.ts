...
export class VersionManager {
  ...
  static parse(versionString: string): VersionInfo {
    ...
    if (month < 1 || month > 12) {
      throw new ValidationError(`Invalid month in version: ${versionString}`, [
        `Month must be between 01 and 12, got: ${monthStr}`,
      ]);
    }

    if (day < 1 || day > 31) {
      throw new ValidationError(`Invalid day in version: ${versionString}`, [
        `Day must be between 01 and 31, got: ${dayStr}`,
      ]);
    }

    if (year < 2020 || year > 2100) {
      throw new ValidationError(`Invalid year in version: ${versionString}`, [
