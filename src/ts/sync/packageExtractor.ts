...
export class PackageExtractor {
    ...
    private async extractCharacters(zip: JSZip): Promise<Character[]> {
        ...
        if (!charactersFile) {
            throw new PackageValidationError(
                'Package missing characters.json',
                'structure'
            );
        }
    ...
    private validateCharacter(character: Character): void {
        ...
        for (const field of requiredFields) {
            if (!(field in character)) {
                throw new PackageValidationError(
                    `Character missing required field: ${field}`,
                    'structure'
                );
            }
        }
        ...
        const validTeams = ['townsfolk', 'outsider', 'minion', 'demon', 'traveller', 'fabled', 'loric', 'meta'];
