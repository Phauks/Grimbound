...
import { loadFonts, type FontSet } from './fontLoader.js'
import {
...
import { parseAbilityText, getTeamColor, type AbilityTextSegment } from './nightOrderUtils.js'
...
async function loadSingleImage(
    ...
    if (cached) {
        console.log(`⚡ ${entry.id}: bytes cache hit (${(performance.now() - startTime).toFixed(0)}ms)`)
        return {
            id: entry.id,
            bytes: cached.bytes,
            type: cached.type,
    ...
    try {
        ...
        if (globalImageCache.has(cacheKey)) {
            ...
            if (bytes) {
                console.log(`🎯 ${entry.id}: global cache hit, converted in ${(performance.now() - cacheStart).toFixed(0)}ms`)
        ...
        console.log(`📍 ${entry.id}: URL resolved in ${(performance.now() - resolveStart).toFixed(0)}ms → ${result.url?.substring(0, 50)}...`)
        ...
        console.log(`✅ ${entry.id}: network fetch completed in ${(performance.now() - fetchStart).toFixed(0)}ms`)
...
async function loadCharacterImages(
    ...
    console.log('🚀 PARALLEL IMAGE LOADING - Batch size:', IMAGE_BATCH_SIZE, 'Total entries:', entries.length)
    ...
    for (let i = 0; i < entries.length; i += IMAGE_BATCH_SIZE) {
        console.log(`🔄 Processing batch ${Math.floor(i / IMAGE_BATCH_SIZE) + 1} of ${Math.ceil(entries.length / IMAGE_BATCH_SIZE)}`)
        ...
        const results = await Promise.allSettled(
            batch.map(entry => loadSingleImage(entry, signal))
        )
...
function drawEntry(
    ...
    // Get team color
    const teamColorHex = getTeamColor(entry.team)
...
export async function generateNightOrderPdf(
    ...
    const renderFirstNight = async () => {
        ...
        renderNightSheet(page, fonts, firstNight.entries, imageMap, 'first', scriptMeta, showScriptName)
    ...
    const renderOtherNight = async () => {
        ...
        const page = pdfDoc.addPage([PAGE_WIDTH_PT, PAGE_HEIGHT_PT])
        renderNightSheet(page, fonts, otherNight.entries, imageMap, 'other', scriptMeta, showScriptName)
