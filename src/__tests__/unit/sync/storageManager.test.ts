...
function triggerError(request: IDBRequest, error?: Error): void {
	queueMicrotask(() => {
		Object.defineProperty(request, 'error', { value: error || new Error('Request failed'), writable: true });
...
function triggerTransactionError(transaction: IDBTransaction, error?: Error): void {
	queueMicrotask(() => {
		Object.defineProperty(transaction, 'error', { value: error || new Error('Transaction failed'), writable: true });
