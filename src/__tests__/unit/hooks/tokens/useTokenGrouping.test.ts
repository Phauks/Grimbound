...
import {
  useTokenGrouping,
  type TokenGroup,
} from "@/hooks/tokens/useTokenGrouping";
...
function createIdentityKey(token: Token): string {
  if (token.type === "reminder") {
    const variantSuffix =
      token.variantIndex !== undefined ? `_v${token.variantIndex}` : "";
...
describe("useTokenGrouping", () => {
  ...
  describe("Token type separation", () => {
    ...
    it("should separate reminder tokens correctly", () => {
      ...
      expect(result.current.reminderTokens[1].reminderText).toBe(
        "Second death",
      );
    });
  ...
  describe("Memoization", () => {
    ...
    it("should update characterTokens reference when tokens change", () => {
      ...
      const tokens2: Token[] = [
        createMockToken({ type: "character", name: "Drunk", order: 0 }),
      ];

      const { result, rerender } = renderHook(
        ({ tokens }) => useTokenGrouping(tokens),
  ...
  describe("Edge cases", () => {
    ...
    it("should handle tokens with special characters in names", () => {
      ...
      expect(result.current.characterTokens).toHaveLength(2);
      expect(result.current.characterTokens[0].name).toBe(
