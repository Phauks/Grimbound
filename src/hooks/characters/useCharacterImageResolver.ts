 ...
 * @module hooks/characters/useCharacterImageResolver
 */
...
export function useCharacterImageResolver({
  ...
  useEffect(() => {
    ...
    const doResolve = async () => {
      ...
      try {
        // Blob URLs are cached by the resolver and managed for the session
        // No cleanup needed - URLs persist and are reused across renders
        const { urls } = await resolveCharacterImages(enrichedCharacters);
    ...
    return () => {
      isMounted = false;
      // Note: No blob URL cleanup needed - URLs are cached for the session
    };
  }, [enrichedCharacters]);
