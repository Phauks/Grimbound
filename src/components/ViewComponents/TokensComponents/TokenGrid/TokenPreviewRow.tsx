...
export function TokenPreviewRow() {
  ...
  const [previewReminderCanvas, setPreviewReminderCanvas] = useState<HTMLCanvasElement | null>(null)
  ...
  const { sampleCharacter, wasAutoSelected, selectedReminderText } = (() => {
    if (characters.length === 0) {
      return { sampleCharacter: SAMPLE_CHARACTER, wasAutoSelected: false, selectedReminderText: null }
    }
  ...
  const generatePreview = useCallback(async () => {
    ...
    try {
      ...
      if (sampleCharacter.reminders && sampleCharacter.reminders.length > 0) {
        ...
        const reminderText = selectedReminderText && sampleCharacter.reminders.includes(selectedReminderText)
  ...
  }, [generationOptions, sampleCharacter, scriptMeta, exampleMetaToken, selectedReminderText, tokens, wasAutoSelected, setExampleCharacterToken])
