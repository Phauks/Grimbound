/**
 * SingleIconSettingsSelector Component
 *
 * A single token type icon positioning control that combines scale, horizontal offset,
 * and vertical offset into a unified component. Used for per-character decorative overrides.
 *
 * Features:
 ...
 *
 * @module components/Shared/SingleIconSettingsSelector
 */
...
export interface SingleIconSettings {
...
export interface SingleIconSettingsSelectorProps {
  /** Current icon settings */
  value: SingleIconSettings;
  /** Called when settings are confirmed (on Apply or panel close) */
  onChange: (settings: SingleIconSettings) => void;
  /** Called on every change for live preview (optional) */
  onPreviewChange?: (settings: SingleIconSettings) => void;
...
const DEFAULT_SETTINGS: SingleIconSettings = {
  scale: 1.0,
  offsetX: 0,
  offsetY: 0,
};
...
export const SingleIconSettingsSelector = memo(function SingleIconSettingsSelector({
  value,
  onChange,
  onPreviewChange,
  displayUnit = 'inches',
  ...
  disabled = false,
  ariaLabel,
}: SingleIconSettingsSelectorProps) {
  // Use the shared expandable panel hook
  const panel = useExpandablePanel<SingleIconSettings>({
    value,
    onChange,
    onPreviewChange,
    disabled,
...
export default SingleIconSettingsSelector;
