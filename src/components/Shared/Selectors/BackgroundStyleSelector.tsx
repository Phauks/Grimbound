...
import { isAssetReference, extractAssetId } from '../../../ts/services/upload/assetResolver'
import { isBuiltInAsset, getBuiltInAssetPath } from '../../../ts/constants/builtInAssets'
import type {
  BackgroundStyle,
  BackgroundBaseMode,
  ...
  useEffect(() => {
    ...
    if (style.effects.vignetteEnabled) {
      ...
      const gradient = ctx.createRadialGradient(
        diameter / 2, diameter / 2, diameter * 0.3,
    ...
    if (style.effects.innerGlowEnabled) {
      ...
      const gradient = ctx.createRadialGradient(
        diameter / 2, diameter / 2, diameter / 2 - glowRadius,
        diameter / 2, diameter / 2, diameter / 2
      )
      gradient.addColorStop(0, 'transparent')
    ...
    ctx.restore()
  }, [style.sourceType, style.mode, style.solidColor, style.gradient, style.effects, imageLoaded, size])
...
export const BackgroundStyleSelector = memo(function BackgroundStyleSelector({
  ...
  const updatePending = useCallback((newValue: BackgroundStyle) => {
  ...
  const handleSourceTypeChange = useCallback((sourceType: BackgroundSourceType) => {
    updatePending({ ...pendingValue, sourceType })
  }, [updatePending, pendingValue])

  const handleModeChange = useCallback((mode: BackgroundBaseMode) => {
  ...
  const handleStyledModeChange = useCallback((mode: BackgroundBaseMode) => {
    updatePending({ ...pendingValue, sourceType: 'styled', mode })
  }, [updatePending, pendingValue])

  const handleSolidColorChange = useCallback((solidColor: string) => {
    updatePending({ ...pendingValue, solidColor })
  }, [updatePending, pendingValue])

  const handleGradientChange = useCallback((gradient: GradientConfig) => {
    updatePending({ ...pendingValue, gradient })
  }, [updatePending, pendingValue])

  const handleTextureChange = useCallback((texture: TextureConfig) => {
  ...
  const handleLightChange = useCallback((light: LightConfig) => {
  ...
  const handlePresetSelect = useCallback((style: BackgroundStyle) => {
    updatePending(style)
  }, [updatePending])
  ...
  const handleImageSelect = useCallback((assetId: string) => {
  ...
  const getSummary = () => {
    ...
    if (currentStyle.sourceType === 'image') {
      ...
      parts.push(`${currentStyle.gradient.type.charAt(0).toUpperCase() + currentStyle.gradient.type.slice(1)} gradient`)
    }
    ...
    // Light adjustments
    const hasLightAdjustments = currentStyle.light &&
        ...
        {pendingValue.sourceType !== 'image' && pendingValue.mode === 'solid' && (
          <div className={drawerStyles.colorRow}>
            ...
            <button
              ...
              onClick={() => handleSolidColorChange(`#${Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0')}`)}
          ...
          <>
            <div className={drawerStyles.colorRow}>
              ...
              <input
                ...
                onChange={(e) => handleGradientChange({ ...pendingValue.gradient, colorStart: e.target.value })}
                className={drawerStyles.colorInput}
                title="Start color"
              />
              <button
                ...
                onClick={() => handleGradientChange({
                  ...
                  colorStart: `#${Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0')}`
                })}
              ...
              <input
                ...
                onChange={(e) => handleGradientChange({ ...pendingValue.gradient, colorEnd: e.target.value })}
                className={drawerStyles.colorInput}
                title="End color"
              />
              <button
                ...
                onClick={() => handleGradientChange({
                  ...pendingValue.gradient,
                  colorEnd: `#${Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0')}`
                })}
            ...
            <div className={drawerStyles.controlRow}>
              ...
              <select
                ...
                onChange={(e) => handleGradientChange({ ...pendingValue.gradient, type: e.target.value as GradientType })}
            ...
            {(pendingValue.gradient.type === 'linear' || pendingValue.gradient.type === 'conic') && (
              <div className={drawerStyles.controlRow}>
                ...
                <input
                  ...
                  onChange={(e) => handleGradientChange({ ...pendingValue.gradient, rotation: Number(e.target.value) })}
        ...
        {pendingValue.sourceType === 'image' && (
          <div className={drawerStyles.imageSelectRow}>
            <div
              ...
              style={pendingResolvedImageUrl ? { backgroundImage: `url(${pendingResolvedImageUrl})` } : undefined}
            />
            <button
              type="button"
      ...
      <div className={drawerStyles.column}>
        ...
        <div className={drawerStyles.controlRow}>
          <button
            ...
            onClick={() => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), brightness: 100 })}
          ...
          <input
            ...
            onChange={(e) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), brightness: Number(e.target.value) })}
            className={drawerStyles.slider}
          />
          <EditableSliderValue
            value={pendingValue.light?.brightness ?? 100}
            onChange={(v) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), brightness: v })}
            min={0}
            max={200}
          />
        </div>
        <div className={drawerStyles.controlRow}>
          <button
            type="button"
            className={drawerStyles.resettableLabel}
            onClick={() => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), contrast: 100 })}
            title="Click to reset to 100%"
            data-label="Contrast"
          >
          ...
          <input
            ...
            onChange={(e) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), contrast: Number(e.target.value) })}
            className={drawerStyles.slider}
          />
          <EditableSliderValue
            value={pendingValue.light?.contrast ?? 100}
            onChange={(v) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), contrast: v })}
            min={0}
            max={200}
          />
        </div>
        <div className={drawerStyles.controlRow}>
          <button
            type="button"
            className={drawerStyles.resettableLabel}
            onClick={() => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), saturation: 100 })}
            title="Click to reset to 100%"
            data-label="Satur."
          >
          ...
          <input
            ...
            onChange={(e) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), saturation: Number(e.target.value) })}
            className={drawerStyles.slider}
          />
          <EditableSliderValue
            value={pendingValue.light?.saturation ?? 100}
            onChange={(v) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), saturation: v })}
            min={0}
            max={200}
          />
        </div>
        <div className={drawerStyles.controlRow}>
          <button
            type="button"
            className={drawerStyles.resettableLabel}
            onClick={() => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), vibrance: 100 })}
            title="Click to reset to 100%"
            data-label="Vibrance"
          >
          ...
          <input
            ...
            onChange={(e) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), vibrance: Number(e.target.value) })}
            className={drawerStyles.slider}
          />
          <EditableSliderValue
            value={pendingValue.light?.vibrance ?? 100}
            onChange={(v) => handleLightChange({ ...(pendingValue.light || DEFAULT_LIGHT_CONFIG), vibrance: v })}
            min={0}
            max={200}
          />
        </div>
        ...
        <div className={drawerStyles.controlRow}>
          <label className={drawerStyles.effectCheckbox}>
            <input
              ...
              onChange={(e) => handleEffectsChange({ ...pendingValue.effects, vignetteEnabled: e.target.checked })}
            />
            Vignette
          </label>
          <input
            ...
            onChange={(e) => handleEffectsChange({ ...pendingValue.effects, vignetteColor: e.target.value })}
            className={drawerStyles.colorInput}
          ...
          <input
            ...
            onChange={(e) => handleEffectsChange({ ...pendingValue.effects, vignetteIntensity: Number(e.target.value) })}
            className={drawerStyles.slider}
            disabled={!pendingValue.effects.vignetteEnabled}
          />
          <EditableSliderValue
        ...
        <div className={drawerStyles.controlRow}>
          <label className={drawerStyles.effectCheckbox}>
            <input
              ...
              onChange={(e) => handleEffectsChange({ ...pendingValue.effects, innerGlowEnabled: e.target.checked })}
            />
            Glow
          </label>
          <input
            ...
            onChange={(e) => handleEffectsChange({ ...pendingValue.effects, innerGlowColor: e.target.value })}
            className={drawerStyles.colorInput}
          ...
          <input
            ...
            onChange={(e) => handleEffectsChange({ ...pendingValue.effects, innerGlowRadius: Number(e.target.value) })}
            className={drawerStyles.slider}
            disabled={!pendingValue.effects.innerGlowEnabled}
            title="Glow radius"
          />
          ...
          <input
            ...
            onChange={(e) => handleEffectsChange({ ...pendingValue.effects, innerGlowIntensity: Number(e.target.value) })}
            className={drawerStyles.slider}
            disabled={!pendingValue.effects.innerGlowEnabled}
      ...
      <div className={drawerStyles.column}>
        ...
        <div className={drawerStyles.controlRow}>
          ...
          <input
            ...
            onChange={(e) => handleTextureChange({ ...pendingValue.texture, intensity: Number(e.target.value) })}
            className={drawerStyles.slider}
        ...
        <div className={drawerStyles.controlRow}>
          ...
          <input
            ...
            onChange={(e) => handleTextureChange({ ...pendingValue.texture, scale: Number(e.target.value) / 100 })}
            className={drawerStyles.slider}
            disabled={pendingValue.texture.type === 'none'}
          />
          <EditableSliderValue
        ...
        <div className={drawerStyles.controlRow}>
          ...
          <select
            ...
            onChange={(e) => handleTextureChange({ ...pendingValue.texture, blendMode: e.target.value as TextureBlendMode })}
            className={drawerStyles.typeSelect}
            disabled={pendingValue.texture.type === 'none'}
          >
        ...
        <div className={drawerStyles.controlRow}>
          ...
          <input
            ...
            onChange={(e) => handleTextureChange({ ...pendingValue.texture, seed: Number(e.target.value) })}
            className={drawerStyles.seedInput}
            disabled={pendingValue.texture.type === 'none' || pendingValue.texture.randomizeSeedPerToken}
          />
          <button
            type="button"
            className={drawerStyles.randomizeButton}
            onClick={() => handleTextureChange({ ...pendingValue.texture, seed: Math.floor(Math.random() * 100000) })}
            disabled={pendingValue.texture.type === 'none' || pendingValue.texture.randomizeSeedPerToken}
          >
            🎲
          </button>
          <label className={drawerStyles.effectCheckbox} title="When enabled, each token gets a unique texture pattern">
            <input
              type="checkbox"
              checked={pendingValue.texture.randomizeSeedPerToken ?? false}
              onChange={(e) => handleTextureChange({ ...pendingValue.texture, randomizeSeedPerToken: e.target.checked })}
              disabled={pendingValue.texture.type === 'none'}
            />
            Unique
          </label>
